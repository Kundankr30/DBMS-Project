...
-- Shibani(Participant & Registration)
CREATE OR REPLACE FUNCTION register_participant(p_name TEXT, p_email TEXT, p_year INTEGER) RETURNS INTEGER AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION view_participants() RETURNS TABLE(id INTEGER, name TEXT) AS $$ BEGIN RETURN QUERY SELECT participant_id, name FROM Participants; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION update_participant(p_id INTEGER, p_name TEXT) RETURNS INTEGER AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION search_participant_by_email(p_email TEXT) RETURNS TABLE(id INTEGER, name TEXT) AS $$ BEGIN RETURN QUERY SELECT participant_id, name FROM Participants WHERE email = p_email; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION delete_participant(p_id INTEGER) RETURNS INTEGER AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION register_for_event(p_pid INTEGER, p_eid INTEGER) RETURNS INTEGER AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION view_registrations() RETURNS TABLE(id INTEGER, p_name TEXT, e_name TEXT) AS $$ BEGIN RETURN QUERY SELECT r.registration_id, p.name, e.event_name FROM Registrations r JOIN Participants p ON r.participant_id = p.participant_id JOIN Events e ON r.event_id = e.event_id; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION view_registrations_by_event(p_eid INTEGER) RETURNS TABLE(id INTEGER, p_name TEXT) AS $$ BEGIN RETURN QUERY SELECT r.registration_id, p.name FROM Registrations r JOIN Participants p ON r.participant_id = p.participant_id WHERE r.event_id = p_eid; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION cancel_registration(p_id INTEGER) RETURNS INTEGER AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION mark_attendance(p_id INTEGER, p_status TEXT) RETURNS INTEGER AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql;
...
--Ashutosh Mohonty  ( -- create performance_judging.sql --- inside that write below functions   --upload schema and below function to ai for help 
CREATE OR REPLACE FUNCTION create_performance(p_rid INTEGER) RETURNS INTEGER AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION view_scores_by_event(p_eid INTEGER) RETURNS TABLE(p_id INTEGER, score DECIMAL) AS $$ BEGIN RETURN QUERY SELECT 1, 0.0::DECIMAL; END; $$ LANGUAGE plpgsql;
...
-- Deepakshi Nayak (awards_managment.sql)   --- inside that write below functions   --upload schema and below function to ai for help 
...
CREATE OR REPLACE FUNCTION assign_award_winner(p_aid INTEGER, p_rid INTEGER) RETURNS INTEGER AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION view_award_winners() RETURNS TABLE(a_name TEXT, p_name TEXT) AS $$ BEGIN RETURN QUERY SELECT a.award_name, p.name FROM Award_Winners aw JOIN Awards a ON aw.award_id = a.award_id JOIN Registrations r ON aw.registration_id = r.registration_id JOIN Participants p ON r.participant_id = p.participant_id; END; $$ LANGUAGE plpgsql;
...
\echo 'Function stubs created for other team members.';