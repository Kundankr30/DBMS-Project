...
-- 2. ASSIGN JUDGE SCORE
DROP FUNCTION IF EXISTS assign_judge_score(INTEGER, INTEGER, DECIMAL, TEXT);
...
-- 3. UPDATE JUDGE SCORE
DROP FUNCTION IF EXISTS update_judge_score(INTEGER, DECIMAL);
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Score record not found';
    END IF;
